...
**Example:** The figure below presents the returns
(i.e. informally speaking the changes in price) for the Starbuck's stock on
...
It can be observed on the left panel that points are not equally spaced.
...
aggregation of points between which it is unable to distinguish anything.
As an example, let us take the data coming from the calibration procedure of
an Inertial Measurement Unit (IMU) which, in general terms, is used to enhance
navigation precision. The signals coming from these instruments are measured at
high frequencies over a long time and are often characterized by linear trends
and numerous underlying stochastic processes. The code below retrieves some 
