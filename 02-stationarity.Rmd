...
\end{aligned}
\] 
4. Just as any covariance, the ${\gamma}\left( {h} \right)$ is "scale dependent", ${\gamma}\left( {h} \right) \in \real$, or $-\infty \le {\gamma}\left( {h} \right) \le +\infty$, so of course we have:
    - if $\left| {\gamma}\left( {h} \right) \right|$ is "close" to zero, then $X_t$ and $X_{t+h}$ are "weakly" (linearly) dependent,
    - if $\left| {\gamma}\left( {h} \right) \right|$ is "far" from zero, then the two random variable present a "strong" (linear) dependence,
    but this is generally difficult to asses what "close" and "far" from zero means in this case. 
...
1. $\rho \left( h \right)$ is scale free so it is much easier to interpret that than $\gamma(h)$.
...
Autocovariances and autocorrelation also turn out to be a very useful tool because they are one of the *fundamental
...
2. \[\begin{aligned}
  \operatorname{var}\left( {{X_t}} \right) &= \operatorname{var}\left( {\sum\limits_{i = 1}^t {{W_t}}  + {X_0}} \right) 
   ...
   = t \sigma_w^2. 
\end{aligned}\] 
where $\sigma_w^2 = \operatorname{var}(W_t)$. Therefore, the variance has a dependence on time contradicting our second property. Moreover, we have:
...
\[ \var \left( {\bar X} \right) = \frac{\sigma_w^2 \left( n - 2\phi - n \phi^2 + 2 \phi^{n + 1}\right)}{n\left(1-\phi^2\right)\left(n \left(\phi - 1\right)^2\right)}\]
