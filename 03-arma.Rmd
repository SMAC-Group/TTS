...
where $|\phi_p| \geq 0$ and $W_t$ is a (Gaussian) white noise process with variance $\sigma^2$. 
```
...
```{definition, label="causalar", name = "Causality of AR models"}
 An AR(p) model is said to be *causal*, if the time series $\{ X_t \}_{-\infty}^{\infty}$ can be written as a one-sided linear process:
\begin{equation}
...
If an AR(p) model is causal if and only if $\phi(z) = 0$ for $|z| \leq 1$. Then, the coefficients of the one-sided linear process given in \@ref(eq:causal) can be obtained by solving
\begin{equation*}
...
Before further discussing the properties of AR($p$) models we consider the following two examples which discuss the causality (and stationarity) of an AR(2).
...
*Step 2:* From Theorem \@ref(thm:theocausalar) we know that if AR process has roots that lie outside the unit circle, we can write $X_t = \phi^{-1}(B)W_t$ and "inverse" the autoregressive operator $\phi(B)$ as follows:
...
\[\phi(z) = 1 - 1.3z + 0.4z^2 = (1 - 0.5z)(1 - 0.8z) ,\]
and the roots of this equation are given by $z_1 = 2 > 1$ and $z_2 = 1.25 > 1$. Moreover, since $z_1$ and $z_2$ are real and distinct, we obtain:
...
There are two ways to derive the ACF for general AR(p) models. 
