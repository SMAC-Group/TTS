...
\[{X_t} = \sum\limits_{i =  - \infty }^\infty  {{\psi _i}{W_{t - j}}} \]

where $W_t \sim WN(0, \sigma^2)$ and $\sum\limits_{i =  - \infty }^\infty  {\left| {{\psi _i}} \right|}  < \infty$.
Note, the later assumption is required to ensure that the series has a limit. 
Furthermore, the set of coefficients \[{\{ {\psi _i}\} _{i =  - \infty , \cdots ,\infty }}\]
can be viewed as linear filter. These coefficients to not have to be all equal
nor symmetric.

Under the **backshift operator** this can be represented as:
