...
From the estimation summary, we note that the confidence interval contains 
values that make the AR(1) non-stationary (i.e. values of $\phi$ larger than 1).
In a larger scope, the estimates are indicating that they represent the
estimated (asymptotic) distribution of $\hat{\phi}$ as displayed in \@ref(fig:asymIC).
...
in order to simulatefrom an AR(1) based on these parameter values. For each 
simulation the parameters are estimated again and saved. Finally, the empirical 
quantiles of the saved  estimated parameter values provide a confidence interval
...
using parametric  bootstrap (using 10000 bootstrap replicates).
...
for(i in seq_len(B)){
  ...
  est.phi.ML[i] = try(arima(x.star, order = c(1,0,0), include.mean = FALSE)$coef, silent = T)
...
hist.phi.ML = hist(na.omit(as.numeric(est.phi.ML)), plot = FALSE, breaks = 20)
...
points(as.numeric(quantile(na.omit(as.numeric(est.phi.ML)), 0.975)), 0, cex = 3, col = colors2[3], pch = "|")
points(as.numeric(quantile(na.omit(as.numeric(est.phi.ML)), 0.025)), 0, cex = 3, col = colors2[3], pch = "|")


hist.phi.GMWM = hist(na.omit(as.numeric(est.phi.gmwm)), plot = FALSE, breaks = 20)
...
points(as.numeric(quantile(na.omit(as.numeric(est.phi.gmwm)), 0.975)), 0, cex = 3, col = colors2[5], pch = "|")
points(as.numeric(quantile(na.omit(as.numeric(est.phi.gmwm)), 0.025)), 0, cex = 3, col = colors2[5], pch = "|")
```

In Figure \@ref(fig:paraAR1cigraphs), we
compare the  estimated densities for $\hat{\phi}$ using asymptotic results and 
bootstrap techniques for the MLE and the GMWM estimator. It can be observed that
the issue that arose previously with unrealistic confidence intervals has been
resolved as the intervals region lie entirely within the boundaries of the 
parameter space.
