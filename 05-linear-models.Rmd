...
yellding the unbiased estimates
...
\[\text{AIC} = \log \left(\hat{\sigma}^2_{\epsilon} \right) + \frac{n + 2p}{n}.\]

This can be derive as follows.
...
\[\KL  =  \frac{1}{n} E_0 \left[\log \left(  
\frac{f (\y| \btheta_0)}
{f (\y| \hat{\btheta})}
\right)\right],\]

where we assume $\btheta_0$ and $\hat{\btheta}$ to denote, respectively, the true parameter vector of interest and an estimator $\btheta_0$ based on a postulated model, and $E_0 \left[\cdot\right]$ denotes the expectation with respect to the the density determined by $\btheta_0$. 


ADDD

For a linear model $\mathcal{M}$ with parameter $\btheta = [\bbeta^T \;\;\; \sigma_{\epsilon}^2]$ we have that its density is given by
...
\[\begin{aligned}
  {E_0}\left[ {\log \left( {\frac{{f\left( {\y|{\btheta_0}} \right)}}{{f\left( {\y|{\hat{\btheta}}} \right)}}} \right)} \right]
   &= {E_0}\left[ {\log \left( {\frac{{{{\left( {\sigma _0^2} \right)}^{ - n/2}}}}{{{{\left( {\sigma ^2} \right)}^{ - n/2}}}}} \right) + \log \left( {\frac{{\exp \left( { - \frac{1}{{2\sigma _0^2}}{{\left( {\y - \X{\bbeta _0}} \right)}^T}\left( {\y - \X{\bbeta _0}} \right)} \right)}}{{\exp \left( { - \frac{1}{{2\sigma ^2}}{{\left( {\y - \X{\hat{\bbeta}}} \right)}^T}\left( {\y - \X{\hat{\bbeta}}} \right)} \right)}}} \right)} \right]  \\
   &= \log \left( {\frac{{{{\left( {\sigma _0^2} \right)}^{ - n/2}}}}{{{{\left( {\sigma ^2} \right)}^{ - n/2}}}}} \right) + {E_0}\left[ {\log \left( {\exp \left( { - \frac{1}{{2\sigma _0^2}}{{\left( {\y - \X{\bbeta _0}} \right)}^T}\left( {\y - \X{\bbeta _0}} \right)} \right)} \right)} \right]  \\
   &- {E_0}\left[ {\log \left( {\exp \left( { - \frac{1}{{2\sigma ^2}}{{\left( {\y - \X{\hat{\bbeta}}} \right)}^T}\left( {\y - \X{\hat{\bbeta}}} \right)} \right)} \right)} \right]  \\ 
   &= -\frac{n}{2}\log \left( {\frac{{\sigma _0^2}}{{\sigma ^2}}} \right) - \frac{1}{{2\sigma _0^2}}{E_0}\left[ {{{\left( {\y - \X{\bbeta _0}} \right)}^T}\left( {\y - \X{\bbeta _0}} \right)} \right] \\
   &+ \frac{1}{{2\sigma ^2}}{E_0}\left[ {{{\left( {\y - \X{\hat{\bbeta}}} \right)}^T}\left( {\y - \X{\hat{\bbeta}}} \right)} \right].  \\ 
\end{aligned} \]

Next, we consider each term separetely,

\[\begin{aligned}
{E_0}\left[ {{{\left( {\y - \X{\bbeta _0}} \right)}^T}\left( {\y - \X{\bbeta _0}} \right)} \right] &= n \sigma^2_0,\\
  {E_0}\left[ {{{\left( {\y - \X{\hat{\bbeta}}} \right)}^T}\left( {\y - Z\hat{\bbeta}} \right)} \right] &= {E_0}\left[ {{{\left( {\y - \left[ {\X \bbeta_0 + \X\left( {\hat{\bbeta} - \bbeta_0} \right)} \right]} \right)}^T}\left( {\y - \left[ {\X \bbeta_0 + \X\left( {\hat{\bbeta} - \bbeta_0} \right)} \right]} \right)} \right]  \\
   &= {E_0}\left[ {{{\left( {\y - \X \bbeta_0 - \X\left( {\hat{\bbeta} - \bbeta_0} \right)} \right)}^T}\left( {\y - \X \bbeta_0 - \X\left( {\hat{\bbeta} - \bbeta_0} \right)} \right)} \right]  \\
   &= {E_0}\left[ {{{\left( {\y - \X \bbeta_0} \right)}^T}\left( {\y - \X \bbeta_0} \right)} \right]  \\
   &- {E_0}\left[ {{{\left( {\y - \X \bbeta_0} \right)}^T}} \right]\X\left( {\bbeta_0 - \hat{\bbeta}} \right) - {\left( {\X\left( {\bbeta_0 - \hat{\bbeta}} \right)} \right)^T}{E_0}\left[ {\left( {\y - \X \bbeta_0} \right)} \right]  \\
   &+ {\left( {\bbeta_0 - \hat{\bbeta}} \right)^T}{\X^T}\X\left( {\bbeta_0 - \hat{\bbeta}} \right)  \\ 
   &= {E_0}\left[ {{{\left( {\y - \X \bbeta_0} \right)}^T}\left( {\y - \X \bbeta_0} \right)} \right] + {\left( {\bbeta_0 - \hat{\bbeta}} \right)^T}{\X^T}\X\left( {\bbeta_0 - \hat{\bbeta}} \right)  \\
   &= {E_0}\left[ {{{\left( {\y - \X\bbeta_0} \right)}^T}\left( {\y - \X\bbeta_0} \right)} \right] + {\left( {\bbeta_0 - \hat{\bbeta}} \right)^T}{\X^T}\X\left( {\bbeta_0 - \hat{\bbeta}} \right)  \\
   &= n\sigma_0^2 + {\left( {\bbeta_0 - \hat{\bbeta}} \right)^T}{\X^T}\X\left( {\bbeta_0 - \hat{\bbeta}} \right).  
\end{aligned} \]

Using these derivation, we obtain

\[\begin{aligned}
  \KL \left(\hat{\btheta}, \btheta_0 \right) & = \frac{1}{n}{E_0}\left[ {\log \left( {\frac{{f\left( {\y|{\btheta_0}} \right)}}{{f\left( {\y|{\hat{\btheta}}} \right)}}} \right)} \right]\\
     &= \frac{1}{n}\left[ {\frac{n}{2}\left( {\frac{{\sigma _0^2}}{{\hat{\sigma}^2}} - \log \left( {\frac{{\sigma _0^2}}{{\hat{\sigma}^2}}} \right) - 1} \right) + \frac{{{{\left( {{\bbeta _0} - {\hat{\bbeta}}} \right)}^T}{\X^T}\X\left( {{\bbeta _0} - {\hat{\bbeta}}} \right)}}{{2\hat{\sigma}^2}}} \right]  \\
   &= \frac{1}{2}\left( {\frac{{\sigma _0^2}}{{\hat{\sigma}^2}} - \log \left( {\frac{{\sigma _0^2}}{{\hat{\sigma}^2}}} \right) - 1} \right) + \frac{1}{2}\frac{{{{\left( {{\bbeta _0} - {\hat{\bbeta}}} \right)}^T}{\X^T}\X\left( {{\bbeta _0} - {\hat{\bbeta}}} \right)}}{{n\hat{\sigma}^2}}  .
\end{aligned} \]


This problem relies heavily on the conclusion given in 2.4 and the
chi-square distribution hint. Specifically, the previous conclusion was:
\[\begin{aligned}
  I\left( {{\beta _1},\sigma _1^2;\hat \beta ,{{\hat \sigma }^2}} \right) &= \frac{1}{n}{E_1}\left[ {\log \left( {\frac{{f\left( {y;{\beta _1},\sigma _1^2} \right)}}{{f\left( {y;\hat \beta ,{{\hat \sigma }^2}} \right)}}} \right)} \right]  \\
   &= \frac{1}{2}\left( {\frac{{\sigma _1^2}}{{{{\hat \sigma }^2}}} - \log \left( {\frac{{\sigma _1^2}}{{{{\hat \sigma }^2}}}} \right) - 1} \right) + \frac{1}{2}\frac{{{{\left( {{\beta _1} - \hat \beta } \right)}^t}{Z^t}Z\left( {{\beta _1} - \hat \beta } \right)}}{{n{{\hat \sigma }^2}}}  \\ 
\end{aligned} \]

Note that
$\frac{{n{{\hat \sigma }^2}}}{{\sigma _1^2}}\sim \chi _{n - k}^2$ and
$\frac{{{{\left( {{\beta _1} - \hat \beta } \right)}^t}{Z^t}Z\left( {{\beta _1} - \hat \beta } \right)}}{{{{\hat \sigma }^2}}}\sim \chi _k^2$.

Proceeding onward: \[\begin{aligned}
  {E_1}\left[ {I\left( {{\beta _1},\sigma _1^2;\hat \beta ,{{\hat \sigma }^2}} \right)} \right] &= {E_1}\left[ {\frac{1}{2}\left( {\frac{{\sigma _1^2}}{{{{\hat \sigma }^2}}} - \log \left( {\frac{{\sigma _1^2}}{{{{\hat \sigma }^2}}}} \right) - 1} \right) + \frac{1}{2}\frac{{{{\left( {{\beta _1} - \hat \beta } \right)}^t}{Z^t}Z\left( {{\beta _1} - \hat \beta } \right)}}{{n{{\hat \sigma }^2}}}} \right]  \\
   &= \frac{1}{2}\left( {{E_1}\left[ {\frac{{\sigma _1^2}}{{{{\hat \sigma }^2}}}} \right] - {E_1}\left[ {\log \left( {\frac{{\sigma _1^2}}{{{{\hat \sigma }^2}}}} \right)} \right] - 1 + {E_1}\left[ {\frac{{{{\left( {{\beta _1} - \hat \beta } \right)}^t}{Z^t}Z\left( {{\beta _1} - \hat \beta } \right)}}{{n{{\hat \sigma }^2}}}} \right]} \right) 
\end{aligned} \]

Using the expectation hint:
\[{E_1}\left[ {\frac{{\sigma _1^2}}{{{{\hat \sigma }^2}}}} \right] = {E_1}\left[ {\frac{n{\sigma _1^2}}{{n{{\hat \sigma }^2}}}} \right]  = n{E_1}\left[ {\frac{{\sigma _1^2}}{{n{{\hat \sigma }^2}}}} \right] = n \cdot \frac{1}{{n - k - 2}} = \frac{n}{{n - k - 2}}\]

Using the independence, expectation, and $X \sim N\left(0,1\right)$ with
$X^2 \sim \chi _1^2$, \[\begin{aligned}
  {E_1}\left[ {\frac{{{{\left( {{\beta _1} - \hat \beta } \right)}^t}{Z^t}Z\left( {{\beta _1} - \hat \beta } \right)}}{{n{{\hat \sigma }^2}}}} \right] &= \frac{1}{n}{E_1}\left[ {\frac{{{{\left( {{\beta _1} - \hat \beta } \right)}^t}{Z^t}Z\left( {{\beta _1} - \hat \beta } \right)}}{{\sigma _1^2}} \cdot \frac{{\sigma _1^2}}{{{{\hat \sigma }^2}}}} \right]  \\
   &= \frac{1}{n}\underbrace {{E_1}\left[ {\frac{{{{\left( {{\beta _1} - \hat \beta } \right)}^t}{Z^t}Z\left( {{\beta _1} - \hat \beta } \right)}}{{\sigma _1^2}}} \right]E\left[ {\frac{{\sigma _1^2}}{{{{\hat \sigma }^2}}}} \right]}_{{\text{Independence}}}  \\
   &= \frac{1}{n}\underbrace {{E_1}\left[ {\frac{{{{\left( {{\beta _1} - \hat \beta } \right)}^t}{Z^t}Z\left( {{\beta _1} - \hat \beta } \right)}}{{\sigma _1^2}}} \right]}_{{X^2}\sim \chi _1^2 \Rightarrow \sum\limits_{i = 1}^k {X_i^2} \sim \chi _k^2
} \cdot \frac{n}{{n - k - 2}}  \\
   &= \frac{1}{n} \cdot k \cdot \frac{n}{{n - k - 2}}  \\
   &= \frac{k}{{n - k - 2}} 
\end{aligned} \]

Returning: \[\begin{aligned}
   &= \frac{1}{2}\left( {{E_1}\left[ {\frac{{\sigma _1^2}}{{{{\hat \sigma }^2}}}} \right] - {E_1}\left[ {\log \left( {\frac{{\sigma _1^2}}{{{{\hat \sigma }^2}}}} \right)} \right] - 1 + {E_1}\left[ {\frac{{{{\left( {{\beta _1} - \hat \beta } \right)}^t}{Z^t}Z\left( {{\beta _1} - \hat \beta } \right)}}{{n{{\hat \sigma }^2}}}} \right]} \right)  \\
   &= \frac{1}{2}\left( {\left( {\frac{n}{{n - k - 2}}} \right) - {E_1}\left[ {\log \left( {\frac{{\sigma _1^2}}{{{{\hat \sigma }^2}}}} \right)} \right] - 1 + \left( {\frac{k}{{n - k - 2}}} \right)} \right)  \\
   &= \frac{1}{2}\left( {\frac{{n + k}}{{n - k - 2}} - {E_1}\left[ {\log \left( {\frac{{\sigma _1^2}}{{{{\hat \sigma }^2}}}} \right)} \right] - 1} \right)  \\
   &= \frac{1}{2}\left( { - {E_1}\left[ {\log \left( {\sigma _1^2} \right) - \log \left( {{{\hat \sigma }^2}} \right)} \right] + \frac{{n + k}}{{n - k - 2}} - 1} \right)  \\
   &= \frac{1}{2}\left( { - \log \left( {\sigma _1^2} \right) + {E_1}\left[ {\log \left( {{{\hat \sigma }^2}} \right)} \right] + \frac{{n + k}}{{n - k - 2}} - 1} \right)
\end{aligned} \]