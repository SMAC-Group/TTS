...
The fourth term
\begin{equation*}
\mathbb{E}[(\bar{X} - \mu)^2] = \frac{1}{T^2} \sum_{t = 1}^{T} \sum_{i = -\infty}^{\infty} \psi_i^2 \mathbb{E}\left[ w_{t-i}^2 \right] = \frac{\sigma^2}{T} \sum_{i = -\infty}^{\infty}\psi_i^2.
\end{equation*}
...
\begin{aligned}
\mathbb{E}|T^{\frac{1}{2}}[\tilde{\gamma} \left( h \right) - \hat \gamma \left( h \right)]| &\leq \frac{T^{\frac{1}{2}}}{T} \sum_{t = T-h+1}^{T} \mathbb{E}|(X_t - \mu)(X_{t+h} - \mu)|\\
&+ \frac{T^{\frac{1}{2}}}{T} \sum_{t = 1}^{T-h} \mathbb{E} |(\bar{X} - \mu)(X_{t+h} - \mu)|\\
&\leq \frac{T^{\frac{1}{2}}}{T^2} Th \sigma^2 \sum_{i = -\infty}^{\infty}|\psi_i \psi_{i+h}| + \frac{T^{\frac{1}{2}}}{T^2} (T-h) \sigma^2 \sum_{i = -\infty}^{\infty}|\psi_i \psi_{i+h}|\\
&= \frac{T^{\frac{1}{2}}}{T^2} (Th + T - h) \sigma^2 \sum_{i = -\infty}^{\infty}|\psi_i \psi_{i+h}|\\
&< \frac{T^{\frac{1}{2}}}{T^2} (Th + T - h) \sigma^2 \left(\sum_{i = -\infty}^{\infty}|\psi_i|\right)^2 \to 0,
\end{aligned}
\end{equation*}
as $T \to \infty$.
...
Since $\{\left( {{X_t} - \mu} \right)\left( {{X_{t + h}} - \mu} \right)\}_1^T$ are iid, we can apply multivariate CLT to it, and have
\begin{equation*}
    \sqrt{T}\left\{
        \begin{bmatrix}
    ...
    - \mathbb{E}\begin{bmatrix}
         ...
         \tilde{\gamma} \left( h \right)
        \end{bmatrix} \right\}
    = \frac{1}{\sqrt{T}}\begin{bmatrix}
         \sum\limits_{t = 1}^{T}(X_t - \mu)^2 - T\mathbb{E}\left[ \tilde{\gamma} \left( 0 \right) \right]\\
         \sum\limits_{t = 1}^{T}\left( {{X_t} - \mu} \right)\left( {{X_{t + h}} - \mu} \right) - T\mathbb{E}\left[ \tilde{\gamma} \left( h \right) \right]
        \end{bmatrix} \overset{\mathcal{D}}{\to} 
    \mathcal{N}\left(0, T \cdot var \left(\begin{bmatrix}
         \tilde{\gamma} \left( 0 \right) \\
         \tilde{\gamma} \left( h \right)
...
\begin{equation*}
    \begin{aligned}
        ...
        &= \mathbb{E}\left[\left[\sum\limits_{t = 1}^{T} {\left( {{X_t} - \mu} \right)^2}\right]\left[\sum\limits_{t = 1}^{T} {\left( {{X_t} - \mu} \right)\left( {{X_{t + h}} - \mu} \right)}\right]\right]\\
        &= 0.
    \end{aligned}
\end{equation*}
