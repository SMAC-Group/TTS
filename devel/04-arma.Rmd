...
\begin{equation}
    X_t = \sum_{j = 0}^{\infty} \psi_j \omega_{t-j} = \psi(B) \omega_t,
    \label{eq:causal}
\end{equation}
...
\begin{equation}
    \pi(B)X_t = \sum_{j = 0}^{\infty} \pi_j X_{t-j} = \omega_t,
    \label{eq:invertible}
\end{equation}
where $\pi(B) = \sum_{j = 0}^{\infty} \pi_j B^j$, and $\sum_{j=0}^{\infty}|\pi_j| < \infty$; we set $\pi_0 = 1$.

It might be difficult and not obvious to show the causality or the invertibility of an ARMA process (especially the higher order process) by using these definitions directly, thus the following properties are useful in practice. Define the *AR and MA polynomials* respectively as
\begin{equation*}
...
- item An ARMA(p, q) is causal iff $\theta(z) \neq 0$ for $|z| \leq 1$,
- item An ARMA(p, q) is causal only when the roots of $\theta(z) = 0$ lie outside the unit circle, i.e. $|z| > 1$.
