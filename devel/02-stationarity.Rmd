...
**Example: MA(1)** Similarly to our previous examples, we attempt to verify the stationary properties for the MA(1) model defined in Section \@ref(moving-average-process-of-order-1):
...
\[\begin{aligned}
  ...
  \var\left( {{y_t}} \right) &= E\left[ {{{\left( {{y_t} - E\left[ {{y_t}} \right]} \right)}^2}} \right] \\
   ...
   &= \underbrace {\phi _1^2\var\left( {{y_{t - 1}}} \right) + \sigma _w^2 = \phi _1^2\var\left( {{y_t}} \right) + \sigma _w^2}_{{\text{Assume stationarity}}} \\
  \var\left( {{y_t}} \right) &= \phi _1^2\var\left( {{y_t}} \right) + \sigma _w^2 \\
  \var\left( {{y_t}} \right) - \phi _1^2\var\left( {{y_t}} \right) &= \sigma _w^2 \\
