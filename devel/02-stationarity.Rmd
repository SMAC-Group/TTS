...
\[\begin{aligned}
  \cov \left( {{X_t},{X_{t+h}}} \right) &= E\left[ {{X_t}{X_{t+h}}} \right] - E\left[ {{X_t}} \right]E\left[ {{X_{t+h}}} \right]
\end{aligned} \]
  
Similarly, the expectations are defined to be:
...
1. The covariance function is **symmetric**. That is, ${\gamma}\left( {h} \right) = {\gamma}\left( -h \right)$ since $\cov \left( {{X_t},{X_{t+h}}} \right) = \cov \left( X_{t+h},X_{t} \right)$.
...
imlying that $\rho_1$ and $\rho_2$ must lie in a parabolic shaped region defined by the above inequalities as illustrated in Figure \ref(fig:admissibility).

```{r admissibility, cache=TRUE, fig.cap="Admissible autocorrelation functions", echo = FALSE}
```
...
\[{\gamma _{XY}}\left( {t,t + h} \right) = \cov \left( {{X_t},{Y_{t + h}}} \right) = E\left[ {\left( {{X_t} - E\left[ {{X_t}} \right]} \right)\left( {{Y_{t + h}} - E\left[ {{Y_{t + h}}} \right]} \right)} \right].\]
...
\[\begin{aligned}
  E\left[ {{X_t}} \right] &= E\left[ {{X_{t - 1}} + {W_t}} \right] 
   ...
   = c \\ 
\end{aligned} \] Observe that the mean obtained is constant since it depends only on the value of the first term in the sequence.
...
Using the *backsubstitution* technique, we can rearrange the AR1 Process so that it is written in a more compact form.

\[\begin{aligned}
  {Y_t} & =  {\phi }{Y_{t - 1}} + {W_t} = \phi \left[ {\phi {Y_{t - 2}} + {W_{t - 1}}} \right] + {W_t} \\
   & =  {\phi ^2}{Y_{t - 2}} + \phi {W_{t - 1}} + {W_t}  \\
   &  \vdots  \\
   & =  {\phi ^k}{Y_{t-k}} + \sum\limits_{j = 0}^{k - 1} {{\phi ^j}{W_{t - j}}} 
\end{aligned} \]

In essence, if we let $k = t$, then we have transformed the AR(1) process into an MA($\infty$).

In addition, we add the requirement that $\left| \phi  \right| < 1$. The origin of the requirement comes from needing to ensure that the characteristic polynomial solution for an AR1 lies outside of the unit circle. Subsequently, stability enables the process to be stationary. If $\phi  \ge 1$, the process would not converge. Under the requirement, the process can represented as a geometric series that converges:
\[\sum\limits_{k = 0}^\infty  {{r^k}}  = \frac{1}{{1 - r}},{\text{ }}\left| r \right| < 1.\]

With this setup, we demonstrate how crucial this property is by calculating each requirement for a stationary equation.

1. First, we will check to see if the mean is stationary. In this case, we opt to use limits to derive the expectation.
\[\begin{aligned}
  \mathop {\lim }\limits_{t \to \infty } E\left[ {{Y_t}} \right] &= \mathop {\lim }\limits_{t \to \infty } E\left[ {{\phi ^t}{Y_0} + \sum\limits_{i = 0}^{t - 1} {\phi ^i{W_{t - i}}} } \right] \\
   &= \mathop {\lim }\limits_{t \to \infty } \underbrace {{\phi ^t}{Y_0}}_{= 0} + \sum\limits_{i = 0}^{t - 1} {\phi ^i\underbrace {E\left[ {{W_{t - i}}} \right]}_{ = 0}}  \\
   &= 0 \\
\end{aligned} \] As expected, the mean is zero and, hence, the first criteria for weak stationarity is satisfied. Note, we used the power of the limit to show that ${\phi ^t}{Y_0} = 0$ as $t \rightarrow \infty$ since $\left| \phi  \right| < 1$.
2. Next, we opt to determine the variance of the process under limits.
\[\begin{aligned}
\mathop {\lim }\limits_{t \to \infty } \var\left( {{y_t}} \right) &= \mathop {\lim }\limits_{t \to \infty } \var\left( {{\phi ^t}{Y_0} + \sum\limits_{i = 0}^{t - 1} {\phi ^i{W_{t - i}}} } \right) \\
   &= \mathop {\lim }\limits_{t \to \infty } \underbrace {\var\left( {{\phi ^t}{Y_0}} \right)}_{ = 0{\text{ since constant}}} + \var\left( {\sum\limits_{i = 0}^{t - 1} {\phi ^i{W_{t - i}}} } \right) \\
   &= \mathop {\lim }\limits_{t \to \infty } \sum\limits_{i = 0}^{t - 1} {\phi ^{2i}\var\left( {{W_{t - i}}} \right)}  \\
   &= \mathop {\lim }\limits_{t \to \infty } \sigma _W^2\sum\limits_{i = 0}^{t - 1} {\phi ^{2i}}  \\
   &= \sigma _W^2 \cdot  
  \underbrace {\frac{1}{{1 - {\phi ^2}}}.}_{\begin{subarray}{l} 
  {\text{Geometric Series}} 
\end{subarray}}
\end{aligned} \] Note, the above result only holds because we are able to use the geometric series convergence as a result of $\left| \phi  \right| < 1$.
3. This leads us to conclude that the autocovariance function is:
\[\begin{aligned}
  \cov\left( {{Y_t},{Y_{t + h}}} \right) &= \cov\left( {{Y_t},\phi {Y_{t + h - 1}} + {W_{t + h}}} \right) \\
   ...
   &= {\phi ^{\left| h \right|}}\frac{{\sigma _W^2}}{{1 - \phi ^2}}. \\ 
\end{aligned} \]

More formally, the autocovariance function can be viewed as a recursion of oneself. That is, we can define it
in terms of its prior value.

\[\begin{aligned}
  \gamma \left( h \right) &= {\phi ^{\left| h \right|}}\frac{{\sigma _W^2}}{{1 - {\phi ^2}}} \hfill \\
   &= \phi \gamma \left( {h - 1} \right) \hfill \\ 
\end{aligned} \]


Both the mean and variance do not depend on time in addition the autocovariance function can be viewed as function dependent on only lags and, thus, the AR(1) process is weakly stationary if $\left| \phi  \right| < 1$.  Lastly, we can obtain the autocorrelation for an AR(1) process. Before we arrive at the final form, note that in a similar vein the autocorrelation
...
After fully simplifying, we arrive at the autocorrelation function for an AR1 being:

\[\rho \left( h \right) = {\phi ^h}\]

Thus, the autocorrelation function for an AR(1) is a _geometric decay_. As in, the smaller the $\phi$, the faster the autocorrelation reaches zero. If $\phi$ is close to 1, then the decay rate is slower.
