...
$${y_t} = {\phi }{y_{t - 1}} + {w_t} \text{, where } {w_t}\mathop \sim \limits^{iid} WN\left( {0,\sigma _w^2} \right)$$

It can be shown that this process simplifies to: 

$$y_t = {\phi ^t}{y_0} + \sum\limits_{i = 0}^{t - 1} {\phi ^i{w_{t - i}}}.$$

In addition, we add the requirement that $\left| \phi  \right| < 1$. This requirement allows the process to be stationary. If $\phi  \ge 1$, the process would not converge.  In this way the process can represented as a geometric series that converges:
...
\[\begin{aligned}
  \mathop {\lim }\limits_{t \to \infty } E\left[ {{y_t}} \right] &= \mathop {\lim }\limits_{t \to \infty } E\left[ {{\phi ^t}{y_0} + \sum\limits_{i = 0}^{t - 1} {\phi ^i{w_{t - i}}} } \right] \\
   &= \mathop {\lim }\limits_{t \to \infty } \underbrace {{\phi ^t}{y_0}}_{= 0 \, \text{since} \, \left| \phi  \right| < 1 \, \text{and} \, t \to \infty} + \sum\limits_{i = 0}^{t - 1} {\phi ^i\underbrace {E\left[ {{w_{t - i}}} \right]}_{ = 0}}  \\
   &= 0 \\
...
Both the mean and autocovariance function do not depend on time and, thus, the AR(1) process is stationary if $\left| \phi  \right| < 1$. 
